...
- **1. userIdentity**  
  ...
  - **Use Case**: Use `userIdentity` to pinpoint if an attacker used stolen IAM credentials or assumed a role to escalate privileges. If `type` is `Root`, escalate immediately due to the high risk of root account compromise.

- **2. userIdentity.accessKeyId**  
  - **Description**: Identifies the access key used in the request.  
  - **Security Importance**: Tracks compromised credentials across multiple events, as attackers often reuse stolen access keys.  
...
- **4. userIdentity.principalId**  
  ...
  - **Use Case**: Track attacker movement across assumed roles or sessions using `principalId`.
...
- **9. userIdentity.sessionIssuer**  
  - **Description**: Identifies the entity Bee issuing session credentials (e.g., a role).  
  - **Security Importance**: Tracks the source of temporary credentials in role-based attacks. If a user make a request with temporary security credentials, sessionIssuer provides information about how the user obtained credentials. For example, if the they obtained temporary security credentials by assuming a role, this element provides information about the assumed role. If they obtained credentials with root or IAM user credentials to call AWS STS GetFederationToken, the element provides information about the root account or IAM user.
...
- **11. eventTime**  
  ...
  - **Use Case**: Sequence attacker actions and determine the duration of unauthorized access. A spike in API calls at unusual times may indicate malicious activity.

- **12. eventName**  
  - **Description**: Specifies the AWS API action (e.g., `GetCallerIdentity`, `CreateAccessKey`, `DeleteBucket`).  
  - **Security Importance**: Reveals attacker techniques, such as reconnaissance (`ListUsers`) or privilege escalation (`CreateAccessKey`).  
...
- **15. userAgent**  
  ...
  - **Security Importance**: Identifies suspicious tools or non-standard applications used by attackers.  
  - **Use Case**: Monitor `userAgent` for anomalies, such as non-AWS SDKs, which may indicate attacker tooling deviating from organizational patterns.

- **16. errorCode and errorMessage**  
  - **Description**: Indicates if an API call failed and why (e.g., `AccessDenied`).  
  - **Security Importance**: Reveals failed attack attempts and permission boundaries, providing insight into an attacker’s scope.  
...
- **17. requestParameters**  
  ...
  - **Security Importance**: Reveals the specific inputs of an API call and configuration details sent by an attacker's actions. 
...
3. **Analyze Actions**: Review `eventName`, `eventSource`, and `readOnly` to understand attacker techniques and affected services.
