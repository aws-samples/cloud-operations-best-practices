...
This guide provides a comprehensive, step-by-step process for assessing migration readiness before transferring an AWS account between AWS Organizations. It combines automated tooling (Account Assessment for AWS Organizations + CFAT) with validated CLI commands to cover all dependencies.

**Applicable for:** Mergers & acquisitions, org consolidation, account restructuring.
...
- [RAM RetainSharingOnAccountLeaveOrganization](https://aws.amazon.com/about-aws/whats-new/2026/02/aws-resource-access-manager/) (Feb 2026) — preserve resource shares during transfer

**References:**
- [AWS Organizations Account Migration Guide](https://docs.aws.amazon.com/organizations/latest/userguide/orgs_account_migration.html)
- [Moving an account - Part 1: Policies, RAM, Condition Keys](https://aws.amazon.com/blogs/mt/aws-organizations-moving-an-organization-member-account-to-another-organization-part-1/)
...
### 2.2 Search Policy Explorer for Org Dependencies
...
**What this finds:**
- Resource-based policies with org conditions (S3, KMS, SQS, SNS, Lambda, etc.)
- Identity-based policies referencing the org
- SCPs with org-specific conditions
...
### 3.1 StackSets Targeting the Account

**Risk:** Service-managed StackSets will DELETE resources from the account when it leaves the org (unless `RetainStacksOnAccountRemoval=true`).
...
- Or document that those resources will be deleted and plan to recreate in target org

### 3.2 IAM Identity Center (SSO) Assignments

**Risk:** All permission set assignments for the migrating account are removed when it leaves. Users lose SSO access to that account.
...
**Action:** Document all assignments. In the target org's Identity Center, recreate equivalent permission sets and assignments after migration.

### 3.3 AWS RAM Resource Shares

**Risk:** Org-scoped RAM shares are revoked when account leaves. New feature allows retention.
...
**Mitigation (Feb 2026 feature):** Enable retention on shares before transfer:
```bash
...
>
> **Tip:** CFAT provides a useful initial snapshot — it confirms whether SCPs, RCPs, Tag Policies, and Backup Policies are enabled at the org level. Use this as a starting point to know which policy types require deeper investigation with the CLI commands below.

```bash
# First: discover all policy types enabled in the org
...
# Get policy content to replicate in target org
...
**Action:** For each policy type enabled in your org:
1. List policies applied to the account (direct + inherited from OU/root)
2. Get policy content with `describe-policy`
3. Replicate in target org BEFORE transfer
4. For declarative policies: verify the account's resources comply with target org policies too
...
1. Register a replacement delegated admin (if org continues)
...
### 3.6 EventBridge Cross-Account Event Bus

**Risk:** Cross-account event delivery permissions referencing the org will break.

```bash
# Check event bus policy for org-based permissions
...
**Action:** Update event bus resource policies to use account IDs instead of org IDs, or add the target org ID.
...
```bash
# List active cost allocation tags (to recreate in target org)
aws ce list-cost-allocation-tags --status Active

# Check CUR report configuration
aws cur describe-report-definitions --region us-east-1

# Check organization details (feature set, for target org compatibility)
...
# Check RI/SP sharing preferences (run from management account)
...
- Re-activate cost allocation tags in target org (takes up to 24 hours)
- **Reserved Instances and Savings Plans:**
  - RIs/SPs **purchased by the migrating account** travel with it — but they will NO LONGER apply to the source org's consolidated bill
  - RIs/SPs **purchased by OTHER accounts** in the source org that were shared with the migrating account will NO LONGER benefit the migrating account after transfer
  - RIs/SPs apply only to the organization where they're purchased — they cannot span multiple organizations
  - If the migrating account was benefiting from org-wide RI/SP sharing, it loses that discount immediately
  - If the migrating account owns RIs/SPs that were shared org-wide, the source org loses that discount capacity
  - **Check sharing mode:** Organization-wide, Prioritized Group, or Restricted Group sharing affects impact scope
  - Contact AWS Support if RIs/SPs need to be transferred to a different account before migration

---

## Phase 4: CFAT Foundation Readiness (Target Org)

Use CFAT results to verify the **target organization** is ready to receive the account:
...
| IAM Identity Center configured | Users need SSO access to the new account |
...
| 8 | RAM shares (owned) | Manual | `get-resource-shares --resource-owner SELF` | ☐ |
| 9 | RAM shares (consumed) | Manual | `get-resource-shares --resource-owner OTHER-ACCOUNTS` | ☐ |
| 10 | RAM RetainSharing enabled | Manual | `update-resource-share --retain-sharing...` | ☐ |
...
| 18 | RI/SP ownership and sharing impact assessed | Manual | `list-savings-plans` + `describe-reserved-instances` | ☐ |
| 19 | Target org age ≥ 7 days | Manual | `describe-organization` | ☐ |
| 20 | Target org account quota | Manual | Check Service Quotas | ☐ |
| 21 | Target org SCPs/RCPs/Declarative policies ready | Manual | Replicate from source | ☐ |
| 22 | Target org Identity Center ready | CFAT | Run CFAT on target | ☐ |
...
**Why this matters:** Once the account leaves the source org:
- Identity Center permission sets are removed → SSO access lost
- `OrganizationAccountAccessRole` trust may break → cross-account access lost
- If no root user credentials exist (common for org-created accounts) → account is locked
...
| 3rd | **Management account (target org)** | Account is now in target org | Use `OrganizationAccountAccessRole` if it exists, or Account Management APIs |
...
| 5 | At least one IAM user/role with admin access exists in the account (independent of org) | ☐ |
...
```bash
# Step 1: From TARGET org management account - send invitation
...
# Accept any RAM share invitations
aws ram accept-resource-share-invitation \
  --resource-share-invitation-arn <INVITATION_ARN> \
  --region <REGION>

# Re-activate cost allocation tags in target org
aws ce update-cost-allocation-tags-status \
  --cost-allocation-tags-status '[{"TagKey": "<KEY>", "Status": "Active"}]'

# Set up Identity Center assignments in target org
...
|--------------------|-------------------|------|------------|
| Resource-based policies (org conditions) | ✅ Policy Explorer | ❌ | — |
| Identity-based policies (org conditions) | ✅ Policy Explorer | ❌ | — |
...
| RAM resource shares | ❌ | ❌ | ✅ `get-resource-shares` |
| RI/SP ownership and sharing impact | ❌ | ❌ | ✅ `list-savings-plans` + `describe-reserved-instances` |
...
| RAM RetainSharingOnAccountLeaveOrganization | Feb 2026 | Resource shares persist through transfer |
| Account Assessment Policy Explorer | v1.1.0+ | Nightly scan for org-dependent policies |
